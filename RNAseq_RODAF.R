...
load("./data/RNAseq_data/Rif_RNAseq_DEseq2_perConditions.RData")
sampleData <- assay(norm_data)
# DESeqDesign <- read.delim(SampleKeyFile, stringsAsFactors=FALSE, sep=",", header=TRUE,  quote="\"", row.names="NAME")
DESeqDesign <- colnames(sampleData) %>% as_tibble() %>%
  mutate(Compound=substring(colnames(RNAseq_norm_dat), 1, 7)) %>%
  column_to_rownames("value") %>% mutate(NAME = rownames(.))
#DESeqDesign <- DESeqDesign[colnames(sampleData),] # To make sure that data and metadata are ordered the same way

Samp4compare<- c("Rif_The", "Rif_Tox") # Experimental group, needs to correspond with the SampleKeyFile. Several condition can be tested c("test1","test2), with the corresponding control
Cont4compare<- c("Control", "Control") # Control group, needs to correspond with the SampleKeyFile
DESIGN<- "Compound"	#Column name samplekeyTEST.csv which defines the groups to be compared

analysisID <-"R-ODAF_Rif"
# # Specify used platform/technology for data generation:
Platform <- "RNA-Seq" # Specify "RNA-Seq" or "TempO-seq"
...
sampleData<- sampleData[,(colSums(sampleData)> minCoverage)] # minCoverage <- 5000000
...
for (x in 1:length(Samp4compare)){	## for all comparisons to be done	
  ...
  res <- results(dds[rownames(compte),], alpha=0.01, cooksCutoff=F, independentFiltering=F, contrast=c(DESIGN[1], condition2, condition1), pAdjustMethod= 'fdr')
